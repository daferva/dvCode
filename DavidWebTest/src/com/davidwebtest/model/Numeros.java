...
public class Numeros extends Matriz{
	
	int matriz[][];
	
	public Numeros(int alto, int ancho, String sContenido) {
		super(alto, ancho, sContenido);
		matriz = new int[this.alto][this.ancho];
		llenarMatriz();
	}
	...
	private void llenarMatriz() {
		...
		for (int f = 0; f < this.alto; f++) {
			for (int c = 0; c < this.ancho; c++) {
				this.matriz[f][c]=Character.getNumericValue(cContenido[i]);
				i++;
			}
		}
	}
			...
			for( int c = 0; c < this.ancho; c++){
				for(int i = 0; i < this.alto; i++){
					for(int j = 0; j <this.ancho; j++){
						if(this.matriz[f][c] < this.matriz[i][j]){
		...
		for (Map.Entry<String, Integer> valor : ocurrencias.entrySet()) {
			if(valor.getValue()>1) {
				...
				for (int f = 0; f < nueva.length; f++) {
					for (int c = 0; c < nueva[f].length; c++) {
						if(valor.getKey().equals(String.valueOf(nueva[f][c])) && cuenta > 1) {
							nueva[f][c] ="*";
							cuenta--;
						}
	...
	private HashMap<String, Integer> elementosUnicosMatriz(){
		...
		for( int f = 0; f < this.alto; f++){
			for( int c = 0; c < this.ancho; c++){
				ocurrencias.put(String.valueOf(this.matriz[f][c]), 0);
			}
		}
		...
		for (Map.Entry<String, Integer> valor : ocurrencias.entrySet()) {
			for (int f = 0; f < this.alto; f++) {
				for (int c = 0; c < this.ancho; c++) {
					if(valor.getKey().equals(String.valueOf(this.matriz[f][c]))) {
	...
	/**
	 * Duplica la matriz actual a una matriz de strings
	 * @return
	 */
	private String[][] duplicarMatriz(){
		String[][] replica = new String[this.alto][this.ancho];
		for (int i = 0; i < this.alto; i++) {
			for (int j = 0; j < this.ancho; j++) {
				replica[i][j]=String.valueOf(this.matriz[i][j]);
			}
		}
		return replica;
	}
